...
public class AndroidAuthenticator implements Authenticator {
    ...
    @Override
    public String getAuthToken() throws com.android.nfRequest.AuthFailureError {
        ...
        try {
            ...
            throw new com.android.nfRequest.AuthFailureError("Error while retrieving auth token", e);
        }
        String authToken = null;
        if (future.isDone() && !future.isCancelled()) {
            if (result.containsKey(AccountManager.KEY_INTENT)) {
                Intent intent = result.getParcelable(AccountManager.KEY_INTENT);
                throw new com.android.nfRequest.AuthFailureError(intent);
            }
            authToken = result.getString(AccountManager.KEY_AUTHTOKEN);
        }
        if (authToken == null) {
            throw new com.android.nfRequest.AuthFailureError("Got null auth token for type: " + mAuthTokenType);
        }
